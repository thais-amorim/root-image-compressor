class RunLenght():
    ...
    def compress(self):
        ...
        for i,b in enumerate(all_bytes,start=1):
            print("index: " + str(i) + " output lenght: " + str(len(output_bits)) + " byte now: " + str(b))
        ...
        print('Final bytes amount: ', len(output_bits))
    ...
    def decompress(self):
        ...
        with open(self.filename, 'rb') as input_file:
            rle = input_file.read()
        print("len of rle: " + str(len(rle)))
        output_bytes = b''
        for i in range(0,len(rle),2):
            print(i)
            rep = int(rle[i])
            value = rle[i+1]
            # print("repetição: " + str(rep))
